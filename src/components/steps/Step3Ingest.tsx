    ...
    <div className="max-w-3xl mx-auto px-4 py-8 step-transition">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 mt-2 text-sm max-w-md mx-auto">
          Paste the JSON output from your AI model. The validator will check the schema and surface any issues.
        </p>
      </div>

      {/* Already have payload banner */}
      {currentPayload && state === 'success' && (
        <div className="mb-6">
          <Alert variant="success" title="Payload Ready">
            ...
            <p className="mt-2 text-xs text-emerald-600">
              You can proceed to Audit or paste a new JSON below to replace this payload.
            </p>
          </Alert>
        </div>
      )}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Paste AI-Generated JSON</CardTitle>
            </div>
            <CardDescription>
              The JSON must contain a <code className="bg-gray-100 px-1 rounded text-xs">stages</code> array of exactly 60 objects and an <code className="bg-gray-100 px-1 rounded text-xs">exams</code> array of exactly 7 objects.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <textarea
              ...
              className="w-full min-h-[400px] px-4 py-3 text-xs font-mono rounded-xl border border-gray-300 bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-y"
              spellCheck={false}
            />
            <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
              <span className="text-xs text-gray-400">
                {jsonText.trim() ? `${jsonText.trim().length.toLocaleString()} characters` : 'Awaiting input...'}
              </span>
              <Button
        ...
        {state === 'json_error' && (
          <Alert variant="danger" title="JSON Parse Error">
            ...
            <p className="mt-2 text-xs">
              Tip: Ensure the JSON starts with <code>{'{'}</code> and ends with <code>{'}'}</code>. Remove any markdown code fences (``` ``` ```) if the AI wrapped the output.
            </p>
          </Alert>
        )}

        {state === 'schema_error' && schemaErrors.length > 0 && (
          <Alert variant="danger" title={`Schema Validation Failed — ${schemaErrors.length} issue${schemaErrors.length > 1 ? 's' : ''} found`}>
            ...
            <p className="mt-3 text-xs">
              Fix these issues in your AI output and re-paste. You can ask the AI to "fix the following schema errors and return only the corrected JSON".
            </p>
          </Alert>
        )}
